...
try:
    ...
    print("Install using: pip install feedparser")
    exit()
try:
    from yattag import Doc, indent
except ImportError:
    print("Error: yattag not installed")
    print("Install using: pip install yattag")
    exit()
try:
    from lxml import html
except ImportError:
    print("Error: lxml not installed")
    print("Install using: pip install lxml")
    exit()
try:
    import requests
except ImportError:
...
with tag('html', dir="rtl"):
    with tag('body'):
        ...
        for item in range(len(d.entries)):  # Loop over articles in RSS feed
            ...
            with tag('p', style="font-family:sans-serif; font-weight:bold;"):
                # Article description
            ...
            with tag('div', id='photo-container'):  # Get picture
                ...
                doc.stag('img', src=imageLink, width="150px",
                         klass="photo")  # Input pic in html
            with tag('p', style='font-family:sans-serif;'):  # Article text
                page = requests.get(d.entries[item].link)
                tree = html.fromstring(page.content)
                # //section[@class='article__entry h-group']/p/child::text()
                textbody = tree.xpath(
                    "//section[@class='article__entry h-group']/p[@class='t-body-text']")
                for item in textbody:
                    raw_text = "{0}".format(
                        item.text_content().encode('iso-8859-1', 'replace'))
